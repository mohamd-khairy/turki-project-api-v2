...
class CallOrderPosNetsuiteApi{

    ///send order netsuite

    public function sendOrderPosToNS(array $data)
    {

        TraceError::create(['class_name'=> "CallOrderPosNetsuiteApi:: coming from the app", 'method_name'=>"sendOrderToNS", 'error_desc' => json_encode($data)]);

        $products = $data['order']['products'];
        $order = $data['order'];
        
    TraceError::create(['class_name'=> "CallOrderPosNetsuiteApi30", 'method_name'=>"sendOrderToNS", 'error_desc' => json_encode($products)]);
        $items = [];
        ...
        TraceError::create(['class_name'=> "CallOrderPosNetsuiteApi40", 'method_name'=>"sendOrderToNS", 'error_desc' => json_encode($items)]);

        $branchId = $order['branch']['reference'];
        TraceError::create(['class_name'=> "CallOrderPosNetsuiteApi41", 'method_name'=>"sendOrderToNS", 'error_desc' => $order['due_at'].'-'.$order['branch']['id']]);

        // $trandate = explode(" ", explode("-", $order['due_at'])[0]) ;
       ...
       $details = [
         ...
         TraceError::create(['class_name'=> "CallOrderPosNetsuiteApi47", 'method_name'=>"sendOrderToNS", 'error_desc' => json_encode($details)]);
         
        $data_string = json_encode($details);

        TraceError::create(['class_name'=> "CallOrderPosNetsuiteApi50", 'method_name'=>"sendOrderToNS", 'error_desc' =>json_encode($data_string)]);
        ...
        TraceError::create(['class_name'=> "CallOrderPosNetsuiteApi::responce", 'method_name'=>"sendOrderToNS", 'error_desc' => json_encode($response)]);
